...
module.exports= function({app, $gulp_folder, gulp, error, $g, $o, $run}){
    ...
    return function(cb){
        gulp.src(['./node_modules/yuidocjs/themes/simple/**/*'])
            ...
            .pipe($g.replace('<div id="classdocs">', '<div id="classdocs"><style>#classdocs .menuli { list-style: none; display: inline-block; border-left: 1px solid black; padding: .5em 1em; } .private_method { color: grey; } .public_method { color: crimson; }</style>'))
            .pipe($g.replace('<li><a href="#', '<li class="menuli"><a href="#'))
            .pipe($g.replace('<li class="menuli"><a href="#methods_{{name}}">{{name}}</a></li>', '<li class="menuli"><a href="#methods_{{name}}">{{name}}</a>{{#if access}} <i class="{{access}}_method">({{access}})</i>{{/if}}</li>'))
            .pipe($g.replace('<div id="methods">', '<div id="methods"><h4>Methods</h4>'))
            .pipe($g.replace('{{#if access}}<em>`/* {{access}} method */`</em>{{/if}}', '<br/>{{#if access}}<strong class="{{access}}_method">/* {{access}} method */</strong>{{/if}}'))
            .pipe($g.replace('{{#if final}} <em>(final)</em>{{/if}}{{#if static}} <em>(static)</em>{{/if}}', '<br/>{{#if access}}<strong class="{{access}}_method">/* {{access}} property */</strong>{{/if}}{{#if final}}/* final */{{/if}}{{#if static}}/* static */{{/if}}'))
